...
// does not close the keepalive descriptor: the OpenStick 410 WWAN driver can
// remove the control endpoint when the final descriptor closes.
type Lease struct {
	entry *entry
	once  sync.Once
}
...
func newCoordinator(opener portOpener) *coordinator {
	return &coordinator{entries: make(map[string]*entry), opener: opener}
}
...
// device node; ensureKeeper detects that inode change and reopens it.
...
// Release allows the next QMI operation to use this control port.
func (lease *Lease) Release() {
	if lease == nil || lease.entry == nil {
		return
	}
	lease.once.Do(func() { lease.entry.gate <- struct{}{} })
}
